<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ taglib prefix="spring" uri="http://www.springframework.org/tags" %>
    
<footer class="footer bg-gradient mt-auto pt-3 cb1 container-fluid shadow">
<div class="row d-flex flex-wrap justify-content-between align-items-center bottom-border-jim g-0">
    <ul class="right-border-jim px-5 col-4">
    <li>
          <a class="text-jim" href="<spring:url value='/index'/>">Home</a>
        </li>
        <li>
          <a class="text-jim" href="<spring:url value='/corsi'/>">Corsi</a>
        </li>
        <li>
          <a class="text-jim" href="<spring:url value='/istruttori'/>">Istruttori</a>
        </li>
        <li>
          <a class="text-jim" href="<spring:url value='/login'/>">Login</a>
        </li>
    </ul>
    <ul class="right-border-jim ps-5 col-4">
     <li>
          <a class="text-jim" href="<spring:url value='/index'/>">Home</a>
        </li>
        <li>
          <a class="text-jim" href="<spring:url value='/corsi'/>">Corsi</a>
        </li>
        <li>
          <a class="text-jim" href="<spring:url value='/istruttori'/>">Istruttori</a>
        </li>
        <li>
          <a class="text-jim" href="<spring:url value='/login'/>">Login</a>
        </li>
    </ul>
    <ul class="ps-5 col-4">
    <li>
          <a class="text-jim" href="<spring:url value='/index'/>">Home</a>
        </li>
        <li>
          <a class="text-jim" href="<spring:url value='/corsi'/>">Corsi</a>
        </li>
        <li>
          <a class="text-jim" href="<spring:url value='/istruttori'/>">Istruttori</a>
        </li>
        <li>
          <a class="text-jim" href="<spring:url value='/login'/>">Login</a>
        </li>
    </ul>
  </div>
  <div class="container-fluid text-center pt-3 text-center">
    <span class="text-jim">Centro Sportivo Big Jim</span>
  	<ul class="list-inline pt-3 pb-0">
      <li class="list-inline-item"><a href="#" class="text-jim">Privacy</a></li>
      <li class="list-inline-item"><a href="#" class="text-jim">Termini</a></li>
      <li class="list-inline-item"><a href="#" class="text-jim">Supporto</a></li>
    </ul>
  </div>
</footer>